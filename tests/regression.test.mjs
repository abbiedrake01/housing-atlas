...
test('regressions use the exact current dataset and separate tenure targets',()=>{
 ...
 for(const tenure of ['LCRA','LCHO']){
  const m=results.models[tenure];assert.equal(m.tenure,tenure);assert.equal(m.points.length,m.n);assert.equal(m.eligible,m.n+m.excluded.length);
  assert.ok(!m.features.some(f=>f.key==='TP01'));assert.equal(m.featureCount,m.features.length);
  if(tenure==='LCHO')assert.ok(!m.features.some(f=>['TP02','TP03','TP04'].includes(f.key)));
  for(const p of m.points){const source=data.providers.find(v=>v.id===p.id);assert.equal(p.actual,source.tenures[tenure].scores[0]);assert.equal(p.stock,source.tenures[tenure].stock);close(p.x[0],Math.log10(p.stock));
   m.features.forEach((f,i)=>{if(f.key.startsWith('TP'))close(p.x[i],source.tenures[tenure].scores[Number(f.key.slice(2))-1]);if(f.kind==='region')assert.equal(p.x[i],Number(source.region===f.label));});
  }
 }
});
test('every held-out provider occurs once and never in its training fold',()=>{
 for(const m of Object.values(results.models)){
  const held=[];
